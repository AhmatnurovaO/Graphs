...
myPainter::myPainter()
{
    front=QBrush(QColor(188,103,108));
    back=QBrush(QColor(235,213,215));
...
void myPainter::draw(QPainter *painter, QPaintEvent *event)                                    //функция дроу в которой происходит отрисоввка элементов
{
    ...
    textPen.setWidth(4);
    extern QVector<TOP> graph1;                                           //вектор объектов топ которые представляют собой веришны
    extern QVector<EDGE> graph2;                                           //вектор объектов эдж которые представляют собой ребра
    for (int i=0;i<graph1.size();i++)
    {
        draw_top(graph1[i].x,                                                  //коррдианты веришны
                 graph1[i].y,
                 QString::number(i+1),
                 painter);
    }
    ...
    {
        draw_edge(graph2[i].x1,                                              //координаты ребра
                  graph2[i].y1,
                  graph2[i].x2,
                  graph2[i].y2,
                  graph2[i].type,
