    ...
    :QMainWindow(parent),ui(new Ui::MainWindow)
{
    ...
    //texts
    G->button(0)->setText("Вершины");
    G->button(1)->setText("<->");
    G->button(2)->setText("Удаление");
    G->button(3)->setText("Отмена");
    btn_calc->setText("Расчитать расстояние");
    //sizes
    G->button(1)->setFixedSize(100,50);
    btn_calc->setFixedSize(200,50);
    ent2->setFixedSize(90,50);
    //locations
    grid->addWidget(openGLW,0,0,8,1);
    grid->addWidget(ent2,1,1);
...
void MainWindow::Fswitch()
{
    ...
    for (int i=0;i<4;i++)
    {
        G->button(i)->setStyleSheet("QPushButton { background-color: rgb(237,207,209); }\n");
    }
    if (buttonID!=4) G->button(buttonID)->setStyleSheet("QPushButton { background-color: rgb(199,112,118); }\n");
    extern QString status;
    extern QString text_for_arrows;
    extern QString statusBook[6];
    if (buttonID==1) text_for_arrows=ent2->text();
...
void MainWindow::F()
{
    ...
    result[0]=' ';
    result[1]=' ';
    result+= " || Результат - "+QString::number(sum);
    QMessageBox::information(this, "Выполнить",result);
}
